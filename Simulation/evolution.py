...
def timeplus1 (dim, H, psi, delta_t):
    ...
    A = np.linalg.inv(np.identity(dim, dtype = complex) + (0. + 1.j)*delta_t/2 * H)
    psi = np.matmul(A, psi)
...
def evolutionCN2 (dim, H0, H1, psi, Gamma, successProbability, gs, t_f, delta_t):
    ...
    for i in gs:
        successProbability[0][iteration] += np.real(psi[i])*np.real(psi[i]) + np.imag(psi[i])*np.imag(psi[i])
   
    successProbability[0][iteration] = successProbability[0][iteration]/float(dim)
...
def evolutionCN3 (dim, H0, H1, psi, Gamma, successProbability, gs, t_f, delta_t):
    ...
    successProbability[0] = np.append(successProbability[0], 0.)
    for i in gs:
        successProbability[0][0] += np.real(psi[i])*np.real(psi[i]) + np.imag(psi[i])*np.imag(psi[i])
...
def evolutionCN5 (dim, H0, H1, psi, Gamma, overlap, gs, t_f, delta_t):
    ...
    for i in range(degeneracy):
        ...
        dot = np.dot(psi, vec)
        prod += (np.real(dot)*np.real(dot) + np.imag(dot)*np.imag(dot))/float(dim)
...
def evolutionCN6 (dim, H0, H1, psi, Gamma, energy, overlap, t_f, delta_t, number_of_overlaps, number_of_eigenstates):
    ...
    for k in range(number_of_eigenstates):
        ...
        dot = np.dot(psi, vec)
        overlap[0][k][0] = (np.real(dot)*np.real(dot) + np.imag(dot)*np.imag(dot))/float(dim)
        energy[0][k][0] = w[k]
...
def evolutionABCN2 (dim, H0, H1, psi, A, B, successProbability, gs, t_f, delta_t):
    ...
    while (t < t_f): 
        ...
        if (iteration%1 == 0):
            successProbability[0] = np.append(successProbability[0], 0.)
            for i in gs:
                successProbability[0][-1] += np.real(psi[i])*np.real(psi[i]) + np.imag(psi[i])*np.imag(psi[i])
...
def evolutionRK2 (dim, H0, H1, psi, Gamma, successProbability, gs, t_f, delta_t):
    ...
    while (t < t_f): 
        ...
        k2 = (0. - 1.j)*np.matmul(H1 + Gamma(t + float(delta_t/2))*H0, psi + float(delta_t/2)*k1)
        k3 = (0. - 1.j)*np.matmul(H1 + Gamma(t + float(delta_t/2))*H0, psi + float(delta_t/2)*k2)
        k4 = (0. - 1.j)*np.matmul(H1 + Gamma(t + float(delta_t))*H0, psi + float(delta_t)*k3)
